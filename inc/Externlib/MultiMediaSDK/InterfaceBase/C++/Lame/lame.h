...
int CDECL lame_set_num_samples(lame_global_flags *, unsigned int);
unsigned int CDECL lame_get_num_samples(const lame_global_flags *);
 ...
 * precision of type 'int'.  Use lame_encode_buffer_long2() instead.
 *
 */
 ...
 * !! NOTE: !! data must still be scaled to be in the same range as
 * type 'int'.   Data should be in the range:  +/- 2^(8*size(int)-1)
 *
 */
 ...
 * final few mp3 frames.  'mp3buf' should be at least 7200 bytes int
 ...
 *
 * 'mp3buf' should be at least 7200 bytes int
 * to hold all possible emitted data.
 *
...
typedef struct {
  ...
  unsigned int  nsamp; /* number of samples in mp3 file.                 */
...
void CDECL id3tag_set_album(
        lame_global_flags*  gfp,
        const char*         album );
void CDECL id3tag_set_publisher(
        lame_global_flags* gfp,
